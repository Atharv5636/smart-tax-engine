...
function ParticleCard({
  ...
  return (
    <Link
      ...
      style={{ "--glow-color": glowColor, backgroundColor: "#120F17" }}
    >
      <div className="magic-bento-card__header">
        <div className="magic-bento-card__label">{item.label}</div>
        <span className="magic-bento-card__corner-icon" aria-hidden>{item.icon}</span>
      </div>
      <div className="magic-bento-card__hero" aria-hidden>
        <span className="magic-bento-card__hero-icon">{item.icon}</span>
      </div>
      <div className="magic-bento-card__content">
        <h2 className="magic-bento-card__title">{item.title}</h2>
        <p className="magic-bento-card__description">{item.description}</p>
        <p className="magic-bento-card__cta">Start Analysis</p>
      </div>
    </Link>
  );
}
...
function MagicBento({
  ...
  const baseClass = `magic-bento-card ${textAutoHide ? "magic-bento-card--text-autohide" : ""} ${
      ...
      <div className="magic-card-grid" ref={gridRef}>
        {items.map((item) =>
          enableStars ? (
            ...
            <Link key={item.to} to={item.to} className={baseClass} style={{ "--glow-color": glowColor, backgroundColor: "#120F17" }}>
              <div className="magic-bento-card__header">
                <div className="magic-bento-card__label">{item.label}</div>
                <span className="magic-bento-card__corner-icon" aria-hidden>{item.icon}</span>
              </div>
