...
function GooeyNav({
  ...
  useEffect(() => {
    const indexFromRoute = items.findIndex((item) => item.href === location.pathname);
    if (indexFromRoute >= 0) {
      setActiveIndex(indexFromRoute);
    }
  ...
  return (
    <div className="gooey-nav-container vertical-gooey" ref={containerRef}>
      <nav>
        <ul ref={navRef}>
